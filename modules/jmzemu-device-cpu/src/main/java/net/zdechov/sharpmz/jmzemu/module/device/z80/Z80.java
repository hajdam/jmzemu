...
public class Z80 {
    ...
    public static int local_tstates;
    ...
    private final int peekb( int addr ) {
        return memoryHandler.read(addr, null);
            // return (m.mz800mode) ? (m.memory.readMem800(addr) & 0xFF) : (m.memory.readMem700(addr) & 0xFF);
    }

    public void pokeb( int addr, int newByte ) {
        memoryHandler.write(addr, (byte) newByte, null);
            // if (m.mz800mode) m.memory.writeMem800(addr, newByte & 0xFF); else m.memory.writeMem700(addr, newByte & 0xFF);
    }
    ...
    public void outb( int port, int bite, int tstates ) {
        portHandler.outPort(port, bite, null);
    }

    public int inb( int port ) {
        return portHandler.inpPort(port, null);
            //return 0xff;
      // return m.io.in(port);
    }
    ...
    public int interrupt(int pioAddr) {
            if (halted==1) {
              nxtpcb();
              halted = 0;
            }
...
//                } // end while
    ...
    void registerMemoryHandler(CpuDeviceModuleApi.MemoryHandler memoryHandler) {
        this.memoryHandler = memoryHandler;
    }

    void registerPortHandler(CpuDeviceModuleApi.PortHandler portHandler) {
